#!/usr/bin/env python3
"""
taskUpSolo - NON-INTERACTIVE Google Drive and Gmail Automation

This is the NON-INTERACTIVE version that auto-processes ALL meetings.
No prompts, no confirmations - just runs automatically.

Usage:
    python3 taskUpSolo.py           # Standalone mode (prepares tasks only)
    python3 taskUpSolo.py claude    # Claude mode (creates tasks via MCP)

Features:
    - Auto-approves all unread meeting notes
    - Auto-confirms task creation
    - Perfect for cron jobs or automated workflows
"""
...
def find_gemini_meeting_notes():
    """
    Step 1: Find and process unread emails with 'Notes:' in subject.

    This captures both direct Gemini meeting notes emails and forwarded meeting notes
    from any sender, making it flexible for various workflows.
    ...
    try:
        ...
        print("\nStep 1: Searching for unread meeting notes emails...")
        print("=" * 60)

        # Search query: unread emails with 'Notes:' in subject (from Gemini or forwarded)
        # This matches both direct Gemini emails and forwarded meeting notes
        query = 'is:unread subject:"Notes:"'
        ...
        if not messages:
            print("No unread meeting notes emails found.")
            return []
...
def shorten_task_name(task_text, assignee=None):
    ...
    # But if it's just "coordinate" without the "to", keep it
    # Example: "Coordinate Insta 360 recording" → "Coordinate Insta 360 recording"
...
def get_workspace_hierarchy_mcp():
    """
    Fetch workspace hierarchy using MCP tools (called from Claude Code context).
    This is a placeholder - the actual MCP call will be made by Claude Code.

    Returns:
        dict: Workspace hierarchy or None if not available
    """
    # This function is meant to be called in Claude Code context with MCP access
    # In standalone mode, we'll return None and fall back to legacy detection
    return None


def ai_analyze_meeting_for_project(meeting_content, meeting_title):
    """
    Use AI to analyze meeting content and determine which project it relates to.
    This is a placeholder - the actual analysis will be done by Claude Code.

    Args:
        meeting_content: Full text of meeting notes
        meeting_title: Title of the meeting

    Returns:
        dict: Suggested project info with confidence score, or None
    """
    # This function is meant to be called in Claude Code context
    # In standalone mode, we'll return None
    return None


def smart_destination_detection(meeting_title, meeting_content=None, workspace_hierarchy=None):
    """
    Intelligently detect the appropriate ClickUp destination based on meeting title and content.

    Strategy:
    1. Option 2 (Primary): Fuzzy match meeting title against workspace hierarchy
    2. Option 3 (Fallback): If no good match, use AI to analyze meeting content
    3. Final Fallback: Use default Operations list

    Args:
        meeting_title: Title of the meeting
        meeting_content: Optional full meeting notes content for AI analysis
        workspace_hierarchy: Optional workspace structure from MCP

    Returns:
        dict: Destination with space_name, folder_name, list_name, list_id, and confidence score
    """
    ...
    # OPTION 2: Fuzzy match against workspace hierarchy
    if workspace_hierarchy:
        print("Strategy: Fuzzy matching against workspace hierarchy...")

        best_match = None
        best_score = 0.0
        threshold = 0.5  # Minimum confidence score

        # Extract key words from meeting title (ignore common words)
        ...
        for space in workspace_hierarchy.get('spaces', []):
            for folder in space.get('folders', []):
                ...
                # Score folder name against title
                folder_score = max([fuzzy_match_score(word, folder_name) for word in title_words] + [0.0])

                # Also check if any title word is contained in folder name
                for word in title_words:
                    if word in folder_name.lower() or folder_name.lower() in word:
                        folder_score = max(folder_score, 0.7)

                for list_item in folder.get('lists', []):
                    list_name = list_item.get('name', '')
                    list_id = list_item.get('id', '')

                    # Score list name against title
                    list_score = max([fuzzy_match_score(word, list_name) for word in title_words] + [0.0])

                    # Also check if any title word is contained in list name
                    for word in title_words:
                        if word in list_name.lower() or list_name.lower() in word:
                            list_score = max(list_score, 0.7)

                    # Combined score (weighted: folder 60%, list 40%)
        ...
        if best_match and best_score >= threshold:
            print(f"✓ Found match: {best_match['folder_name']} → {best_match['list_name']}")
            print(f"  Confidence: {best_score:.2%}")
            return best_match
        else:
            print(f"✗ No confident match found (best score: {best_score:.2%})")

    # OPTION 3: AI analysis of meeting content (fallback)
    if meeting_content:
        print("Strategy: AI analysis of meeting content...")
        ai_result = ai_analyze_meeting_for_project(meeting_content, meeting_title)

        if ai_result and ai_result.get('confidence', 0) > 0.6:
            print(f"✓ AI identified project: {ai_result.get('folder_name')} → {ai_result.get('list_name')}")
            print(f"  Confidence: {ai_result['confidence']:.2%}")
            return ai_result
        else:
            print("✗ AI analysis did not produce confident result")

    # FINAL FALLBACK: Default to Operations list
    print("⚠️  Using default fallback: Operations list")
    return {
        'space_name': 'Clients',
...
def preview_clickup_tasks(action_items, meeting_title, destination=None):
    ...
    print("=" * 60)
    print("\n✓ AUTO-CONFIRMED: Tasks will be created automatically")

    return True  # Always confirm in non-interactive mode
...
def create_clickup_tasks(action_items, destination, meeting_title):
    ...
    for i, item in enumerate(action_items, 1):
        ...
        # Build task object for ClickUp (use shortened task name as title)
        task_obj = {
            'name': item['task'],
...
def prompt_for_meeting_approval(emails):
    """
    AUTO-APPROVE: Automatically approve all meetings for processing (non-interactive).

    Args:
        emails: List of email dicts with meeting info

    Returns:
        list: Indices of all emails (auto-approved)
    """
    print(f"\n{'='*60}")
    print(f"FOUND {len(emails)} UNREAD MEETING NOTE(S) - AUTO-PROCESSING ALL")
    print(f"{'='*60}\n")
    ...
    print(f"{'='*60}")
    print(f"✓ AUTO-APPROVED: All {len(emails)} meeting(s) will be processed")
    print(f"{'='*60}\n")

    # Auto-approve all meetings
            ...
            for idx in approved_indices:
                ...
                if email['meeting_notes_link']:
                    ...
                    if content:
                        ...
                        # Smart destination detection (pass content for AI fallback)
                        destination = smart_destination_detection(meeting_title, meeting_content=content)
