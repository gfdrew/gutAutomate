#!/usr/bin/env python3
"""
gutAutomate Standalone Edition - Google Drive and Gmail Automation

Creates ClickUp tasks directly via REST API.
No Claude/MCP dependencies required.

Usage:
    gut-automate                     # Interactive mode
    gut-automate --batch             # Batch mode (auto-approve)
"""
...
def parse_action_items(notes_content, meeting_title="", debug=False):
    ...
    print("=" * 60)
    print("Using regex-based parsing (Standalone Edition)")
...
def select_best_list_for_task(lists, task_text, context, meeting_title, primary_list_name=None):
    ...
    # Standalone Edition: Use primary list or most recently updated
    # (Claude API consultation not available in standalone)
...
def preview_clickup_tasks(action_items, meeting_title, destination=None):
    ...
    Standalone Edition - always prompts for user confirmation.
...
def create_clickup_tasks_via_api(action_items, destination, meeting_title):
    """
    Step 5: Create tasks in ClickUp using REST API.
    ...
    for i, item in enumerate(action_items, 1):
        ...
        # Add due date if available (REST API expects integer)
    ...
    print(f"\n✓ Prepared {len(tasks_to_create)} tasks for ClickUp REST API creation")
    print(f"  List IDs: Multiple (per-task routing)")

    # Standalone mode - return the prepared tasks for REST API creation
    return {
...
def create_clickup_tasks(action_items, destination, meeting_title):
    """
    Legacy wrapper for create_clickup_tasks_via_api.
    Kept for backwards compatibility.
    """
    return create_clickup_tasks_via_api(action_items, destination, meeting_title)


def prompt_for_meeting_approval(emails):
    """
    Show list of meetings and prompt user to select which ones to process.
    Always interactive (Standalone Edition).
...
def main(mode='standalone'):
    """
    Main entry point for gutAutomate Standalone Edition.

    Always runs using ClickUp REST API directly.
    No Claude/MCP dependencies.

    Args:
        mode: Must be 'standalone' (kept for compatibility)
    """
    print("🚀 gutAutomate Standalone Edition")
    print("   Using: ClickUp REST API (direct)")
    print()
            ...
            if not meetings_data:
                ...
                for idx, meeting in enumerate(meetings_data):
                    ...
                    if should_create:
                        ...
                        # Prepare tasks for ClickUp REST API
                        result = create_clickup_tasks_via_api(
                            meeting['action_items'],
                            meeting['destination'],
                            meeting['meeting_title']
                        )
